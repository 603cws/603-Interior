...
function SelectArea({
  ...
  const handleAddonClick = () => {
    ...
    setSelectedData((prevData) => {
      return prevData.map((item) => {
        if (item.id === selectedProductView.id) {
          // Merge old and new addons for this specific product
          const existingAddons = selectedAddonsMap[currentGroupKey] || [];
          const mergedAddons = [
            ...existingAddons.filter(
              (a) => !selectedAddons.some((b) => b.id === a.id)
            ),
            ...selectedAddons,
          ];
    ...
    // Update the selectedAddonsMap for this product
    setSelectedAddonsMap((prev) => ({
      ...prev,
      [currentGroupKey]: [...(prev[currentGroupKey] || []), ...selectedAddons],
    }));
  ...
  const handleAddonSelect = (addon, isChecked) => {
    ...
    setSelectedAddonsMap((prev) => {
      ...
      // Add or remove the addon only for this specific product
    ...
    // Also update selectedAddons state only for the current product
