...
function ProductCard({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pb-8 pt-3">
      {loading ? (
        Array.from({ length: 4 }).map((_, index) => (
          <div
            key={index}
            className="w-11/12 h-full flex flex-col justify-center items-center bg-white rounded-lg shadow-md my-2 px-3"
          >
            {/* Image Skeleton */}
        ...
        filteredProducts.map((variant) => (
          ...
          >
            ...
            <div className="w-full aspect-[4/3] rounded-t-lg relative">
              ...
              <img
                className={`object-contain rounded-t-lg w-full h-full transition-opacity duration-300 ${
                  loadingImages[variant.id] !== false
                    ? "opacity-0"
                    : "opacity-100"
                }`}
