...
function VendorProfile({ setIsEditopen }) {
  const { accountHolder } = useApp();
  const [profileImage, setProfileImage] = useState(null);
  useEffect(() => {
    setProfileImage(accountHolder.profileImage);
  }, [accountHolder.profileImage]);

  return (
    <div className="sm:w-[400px] lg:w-[520px] shadow-2xl   rounded-3xl">
      ...
      <div className="text-sm px-10 lg:mp-16  pb-4 lg:pb-8 pt-2 p-5 w-full ">
        <div className="flex justify-center  items-center">
          <img src={profileImage} alt="profile" className="w-28 h-28" />
        </div>
        ...
        <div className="flex   items-center gap-4 text-nowrap my-2">
          <h3 className="text-[#CACED8]  capitalize flex-1">Phone Number</h3>
          <p className="text-[#194F48] flex-1">{accountHolder.phone}</p>
        </div>
        <div className="flex justify-center  gap-4 text-nowrap my-2">
          <h3 className="text-[#CACED8]  capitalize flex-1">Company Name</h3>
